...
def get_by_user_id_filter_and_limit(user_id: int, limit: int) -> list[ChatLog] | None:
    ...
    if UserDomain.is_valid_id(user_id):
        queryset = ChatLog.objects.filter(user__id=user_id, include_in_future=True).order_by("-last_modified")[:limit]
    return queryset
...
def create_chat_log(user: User, role_type: str, message: str, include_in_future: bool) -> ChatLog:
    return ChatLog.objects.create(user=user, role_type=role_type, message=message, include_in_future=include_in_future)
